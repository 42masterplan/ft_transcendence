...
export default function ChangeAvatar({beforeAvatar}: {beforeAvatar: string}) {
  ...
  useEffect(() => {
    if (beforeAvatar != null && !avatarList.includes(beforeAvatar)) {
      setAvatarList([...avatarList, customAvatar]);
      setProfileImage(beforeAvatar);
      setCustomAvatar(beforeAvatar);
      setSelected(15);
    }
  }, [beforeAvatar]);
  useEffect(() => {
  ...
  return (
    <div className={`w-full ${hoverEffect}`}>
      <h1 className={subtitleFontStyle}>아바타 선택</h1>
