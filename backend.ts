...
const gameStates: {[key: number]: State} = {};
 ...
 */
function createNewGameState(gameId: number) {
  return {
    gameId: gameId,
    ready: false,
    players: [
...
function resetBall(isA: boolean, state: State, io: any) {
  ...
  setTimeout(() => {
    ...
    const y = !isA ? state.players[0].y : state.players[1].y;
...
nextApp.prepare().then(() => {
  ...
  io.on('connection', (socket: any) => {
    ...
    socket.on('disconnect', (reason: any) => {
      ...
      const gameId = Object.keys(gameStates).find((id) => {
        const state = gameStates[parseInt(id)];
      ...
      // 게임 룸을 찾지 못하면 에러를 출력합니다. (로직상 불가능합니다 ;)..
      if (!gameStates[numId].ready) {
        // 1명이 들어왔는데 두 번째 플레이어가 들어오기 전에 연결이 끊긴 경우 게임 상태를 삭제합니다.
        delete gameStates[numId];
        return;
      }
      // 플레이어 A가 연결을 끊으면 플레이어 B가 기권패합니다.
      const state = gameStates[numId];
    ...
    socket.on('keyDown', (keycode: string) => {
      ...
      if (!gameId) return;
      const targetPlayer = gameStates[parseInt(gameId)].players.find(
  ...
  setInterval(() => {
    ...
    Object.keys(gameStates).forEach((gameId) => {
      const state = gameStates[parseInt(gameId)];
